      ...
      <div className="container mx-auto px-6">
        ...
        {loading ? (
          <div className="flex flex-col justify-center items-center py-20 gap-4">
            ...
            <p className="text-gray-600 dark:text-gray-400 font-rajdhani uppercase tracking-widest">Loading Team...</p>
          </div>
        ) : (
          <motion.div 
              ...
              {team.map((member) => (
                <SwiperSlide key={member.id}>
                  <div className="bg-white/40 dark:bg-zinc-900/60 backdrop-blur-md p-8 rounded-sm shadow-sm dark:shadow-md border border-red-600/10 dark:border-red-500/20 flex flex-col items-center text-center group transition-all hover:shadow-xl dark:hover:shadow-red-500/20 hover:border-red-600/50 dark:hover:border-red-500/40">
                    ...
                    <h4 className="font-orbitron text-xl font-bold uppercase tracking-tighter text-gray-900 dark:text-white mb-6">
                      {member.name}
                    </h4>

                    <div className="flex items-center gap-4 mt-auto">
                      {member.github && (
                        <a href={member.github} target="_blank" rel="noopener noreferrer" className="text-gray-400 dark:text-gray-500 hover:text-red-600 dark:hover:text-red-500 transition-colors">
                          <Github size={20} />
                        </a>
                      )}
                      {member.linkedin && (
                        <a href={member.linkedin} target="_blank" rel="noopener noreferrer" className="text-gray-400 dark:text-gray-500 hover:text-red-600 dark:hover:text-red-500 transition-colors">
                          <Linkedin size={20} />
                        </a>
                      )}
                    </div>
